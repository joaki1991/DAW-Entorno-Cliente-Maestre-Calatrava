$(() => {
  ...
  function testMultipleSelection1() {
    ...
    const liElements = $('footer div.row li');
  ...
  function testCssMethod() {
    ...
    const othersElements = $('header div.row li, #categories img');
    othersElements.css('border', '1px solid green');

    $$result.logBold('Método index()');
    const images = $('#categories img');
    const img1 = $('#categories img:eq(1)');
  ...
  function testFilter() {
    ...
    $('i').filter('.bi-chevron-right').css('border', '1px solid red');
    ...
    images.filter((index) => (index > 0 && index < images.length - 1)).attr({ src: 'https://via.placeholder.com/258x172.jpg?text=Filtrado2' });
  ...
  function testNot() {
    ...
    $('i').not('.bi-chevron-right').css('border', '1px solid red');
  ...
  function testMap() {
    ...
    const idElements = $('*[id]');
    const map = idElements.map(function (index) {
      return this.id;
    });
