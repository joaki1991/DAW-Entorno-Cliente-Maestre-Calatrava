(function () {
  ...
  function iteratorProtocol() {
    ...
    for (const value of arr.entries()) { // Obtiene array con posición y el valor
  ...
  function nativeIterators() {
    ...
    mapExample.set(2, 'b');
    mapExample.set(3, 'c');
    const mapIterator = mapExample[Symbol.iterator]();
    ...
    for (const value of strIterator) {
      $$result.log(value);
    }

    $$result.logBold('Iteradores siendo iterables');
    const iterator = [1, 'a', 3, 'd', 5].entries();
  ...
  function buildIteratorAndIterable() {
    ...
    const iterable2 = myIterator(2, 5);
  ...
  function iterableList() {
    class List {
      ...
      remove(position) {
        if (position >= this.#list.length || position < 0) { throw new Error('Intento de borrado fuera del límite de la lista.'); }
    ...
    const iterable = list.elements; // Obtenemos el iterable de la lista.
    ...
    for (const element of iterable) { // Iteramos sobre el iterable
      $$result.log(`Objeto: ${element.property1} ${element.property2}`);
    }
  }
