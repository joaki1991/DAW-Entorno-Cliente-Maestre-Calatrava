...
const Manager = (function () {
  ...
  class Manager {
    ...
    * getProductsByTypeAndCategory(type, category, field) {
      const typeCondition = (type)
        ? (product) => product instanceof type
        : () => true;
      let categoryCondition; let
        storedCategory;
      if (category) storedCategory = this.#categories.get(category.title);
      if (storedCategory) {
        categoryCondition = (product) => storedCategory.products.has(product.serial);
      } else {
        categoryCondition = () => true;
      }

      if (field) {
        const array = [...this.#products.values()].filter(
          (product) => typeCondition(product) && categoryCondition(product),
        );
        if (this.#order[field]) {
          array.sort(this.#order[field]);
        }
        ...
        for (const product of this.#products.values()) {
          if (typeCondition(product) && categoryCondition(product)) {
            yield product;
          }
        }
      }
    ...
    getCategory(title = 'Anon', url = 'https://via.placeholder.com/258x172.jpg?text=SinNombre') {
    ...
    getProduct(serial, brand, model, price, type) {
      ...
      if (!product) {
        product = new this.#productConstructors[type](serial, brand, model, price);
      }
      return product;
    }
  }
