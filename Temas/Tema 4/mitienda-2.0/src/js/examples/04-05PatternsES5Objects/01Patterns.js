(function () {
  ...
  const divExamples = examples.children[11];
  ...
  function testPatternIIFE() {
    ...
    const app = (function () { // Función principal anónima
      const carId = 123; // Variable con ámbito local solo accesible desde la función anónima y por tanto encapsulada.
    ...
    const Book = (function () { // Utilizamos una función anónima.
      ...
      function checkIsbn(isbn) {
        if (!(/^(\d{3}-\d{2}-\d{5}-\d{2}-\d)$/.test(isbn)
					|| /^(\d-\d{6}-\d\d-\d)$/.test(isbn))) { throw new TypeError('isbn is not valid!'); }
      }
  ...
  function testAbstractType() {
    ...
    try {
      ...
      const myForm = new BaseForm(); // Al intentar instaciar se genera la excepción.
    } catch (err) {
  ...
  function testInterface() {
    ...
    Interface.checkImplements = function (obj, interfaceObj) {
      for (const method of interfaceObj.methods) { // Recorremos el array de métodos
    ...
    const iMaterial = new Interface('IMaterial', ['getName', 'getPrice']);
  ...
  function testSingletonPattern() {
    const Singleton = (function () { // La función anónima devuelve un método getInstance que permite obtener el objeto único
      ...
      function init() { // Este método se ejecuta una única vez y es el utilizado para crear la única instancia del objeto.
        ...
        return { // Devuelve el objeto que será único. Funciona como un clousure
          ...
          getRandomNumber() { // Propiedad que permite acceder a los atributos privados. Closure
  ...
  function testSingletonV1() {
    ...
    const Logger1 = { // Definimos el objeto
      enabled: true, // La propieda enabled dice si se puede ejecutar el método o no.
      log(logText) { // Asociamos un método al objeto
        ...
        { console.log(logText); } else // Si no existe se muestra la alerta en la página.
        { alert(logText); }
      },
    };
  ...
  function testIteratorSchema() {
    ...
    const iterator1 = myIterator(2, 5);
    ...
    const iterator2 = myIterator(2, 5);
  ...
  function testIteratorPattern() {
    ...
    function List() {
      ...
      if (!(this instanceof List)) { throw new Error('Debes invocar este constructor con el operador new'); }
      ...
      this.remove = function (position) {
        if (position >= _list.length || position < 0) { throw new Error('Intento de borrado fuera del límite de la lista.'); }
      ...
      Object.defineProperty(this, 'iterator', {
        get() {
          ...
          return {
            next() { // Método del iterador que devuelve el siguiente objeto de la lista.
    ...
    const { iterator } = list; // En un variable obtenemos un iterador sobre la lista.
