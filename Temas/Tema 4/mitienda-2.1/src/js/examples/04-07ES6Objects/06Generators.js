(function () {
  ...
  function generatorSyntax() {
    ...
    const it = datesGenerator(); // Obtenemos un iterador e iterable
  ...
  function generatorAsIterator() {
    ...
    const it = datesGenerator(5); // Obtenemos un iterador e iterable
  ...
  function trhowMethod() {
    ...
    while (!elem.done) {
      $$result.log(elem.value); // Fecha actual incrementada por un múltiplo de 10
      if (i === 2) { it.throw(new Error('Error: Alcanzado el límite de 2.')); } // Error: Alcanzado el límite de 2.
      elem = it.next(++i * 10);
    }
  ...
  function iterableList() {
    ...
    for (const element of list.elements()) { // Iteramos sobre el iterable
      $$result.log(`Objeto: ${element.property1} ${element.property2}`);
    }

    $$result.logBold('List como iterador');
    for (const element of list) { // Iteramos sobre el iterable
      $$result.log(`Objeto: ${element.property1} ${element.property2}`);
    }
  }
  ...
  function recursiveGenerator() {
    ...
    for (const element of externalGenerator()) {
